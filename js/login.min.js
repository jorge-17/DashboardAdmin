var user;var pass;var datos;var resultado;var resultadoP;const rootURL="http://localhost:26010/Dashboard.svc/";const autenticacion="Autenticacion";const permisos="ObtenerPermisos";$("#btnLogin").on('click',async function(){user=$("#user").val();pass=$("#pass").val();try{resultadoP=await $.ajax({type:"POST",async:!1,url:rootURL+permisos,data:JSON.stringify({usuario:user,password:pass}),contentType:"application/json;charset=utf-8",dataType:"json"})}catch(error){console.log(error)}
    ...
    $.when(resultado).then(function(data){datos=JSON.parse(data.d);var item=datos.aoData;if(item!=null){var tokenS=item.split("|");sessionStorage.setItem('token',tokenS[0]);sessionStorage.setItem('NombreU',tokenS[1]);window.location.replace("http://localhost/dashboard/views/home.html")}else{new PNotify({title:'Datos incorrectos',text:'Verifica que las credenciales sean correctas',type:'error',styling:'bootstrap3'})}})})